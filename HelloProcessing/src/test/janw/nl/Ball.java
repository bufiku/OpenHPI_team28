 ...
 * Open.HPI.de course "Java Capstone series 1." This class is responsible for
 * the ball; it has attributes for x- and y-position, the speed and for
 * convenience its speeds along the x-axis and y-axis (using sin/cos of the
 * angle and the speed) This class has methods for creating, moving and drawing
 * of the ball It has private methods for setting the speed, and bouncing
 * against the walls, ceiling or the bat.
 *
 * What I have doubts about, is the fact that the ball is aware of the canvas
 * (subclass of PApplet of the processing framework). I would like it if the
 * ball was loosely coupled, without a hard reference.
 *
 * ToDo: For now, if you miss the ball there will only be a
 * System.out.println()-message. That should be made nicer. My current idea is
 * to raise either an exception; or to raise an event, so I will learn something
 * about catching events. Furthermore, I think it would be nice to refactor the
 * whole solution to a model-view-controller. However, as the deadline of the
 * course approaches, and the rest of the team does not seem to be very active,
...
public class Ball {
	 ...
	 * speed and starting angle). Finally it is being told its size in radius.
	 */
	...
	/**
	 * Method move: This method moves the ball. Furthermore, it checks if the ball
	 * hits the ceiling, one of the walls, or the bat/paddle. If it is at the same
	 * height as the bat/paddle, but doesn't hit it for now it generates a message
	 * that you lose, currently as a System.out.println()-message. Furthermore, as a
	 * temporarily solution, the ground reacts as a boundary as well, so the ball
	...
	public void move() {
		...
		// + " y_as " + y_as + " y_speed " + y_speed + " batyaxis " +
		// playField.getBat().getY_as() );
		...
		if (y_as <= (radius / 2)) { // Is it hitting the upper ceiling // For breakout check against the array of
									// bricks
			bounceHorizontal();
		}
